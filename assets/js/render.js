...
window.AxisRender = (function () {
  ...
  // Resolves a hash from a scene's conflict.opponents list (or any other
  // adversary/NPC reference) to a normalized { kind, name, properties,
  // features } row — regardless of whether the hash points at the
  // adversary catalog, an NPC's stat-block DEF, or an NPC profile box that
  // is linked_to its own stat-block DEF (the profile/statblock split
  // npcs.js documents at the top of that file). Scene opponent lists
  // reference the PROFILE hash for named cast members (e.g. "The Howling
